...
- **Post-create config append:** after **`pct create`**, compose **`env`** / **`lxc_config_lines`** are written to **`/etc/pve/lxc/<vmid>.conf`** via a temp file and in-place overwrite (not **`mv --preserve`**, which spuriously warns on **pmxcfs**). If you still see `mv: failed to preserve ownership for '/etc/pve/lxc/…'` on an older checkout, the lines were usually applied anyway; upgrade **`lib/oci-create.inc.sh`** to drop the noise.

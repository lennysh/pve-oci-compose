...
| `env` / `environment` | optional | Same field: Docker-style **`environment`** is an alias for **`env`**. Values become **`pct --env`**, which expects **one** NUL-separated `KEY=value` list; the driver merges all entries accordingly. YAML **mapping** (`KEY: value`), **list** of `KEY=value` strings, a single string, or a list of one-key objects / `{name:, value:}` objects (Kubernetes-style). |

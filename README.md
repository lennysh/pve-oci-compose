...
| `env` / `environment` | optional | Same field: Docker-style **`environment`** is an alias for **`env`**. YAML **mapping** (`KEY: value`), **list** of `KEY=value` strings, a single string, or a list of one-key objects / `{name:, value:}` objects (Kubernetes-style). **OCI vztmpl:** Proxmox sets the CT **`env`** from the image manifest during unpack and **overwrites** **`pct create --env`**, so this project appends each `KEY=value` as **`lxc.environment.runtime:`** after **`pct create`** (in the same marked block as **`lxc_config_lines`**), so compose variables actually reach the guest. |
...
| `lxc_config_lines` | optional | List of strings appended **after** a successful **`pct create`** into **`/etc/pve/lxc/<vmid>.conf`** (low-level **`lxc.*`** / **`lxc.mount.entry`** style lines, **`KEY: value`** or **`KEY = value`**). Compose **`env`** / **`environment`** is written in the **same** append pass as **`lxc.environment.runtime:`** (see **`env`** row above). No **`#` comment markers** are used in the conf file — Proxmox surfaces **`#` lines from `.conf` in the CT Notes UI. Only runs when **apply** actually creates the CT (not when the vmid already exists). |
...
- **Post-create config append:** after **`pct create`**, compose **`env`** / **`lxc_config_lines`** are appended to **`/etc/pve/lxc/<vmid>.conf`** (no **`#` markers** — Proxmox copies **`#` comment lines from the conf into the CT Notes field). If Notes still show `BEGIN pve-oci-compose lxc_config_lines` from an older run, edit **`/etc/pve/lxc/<vmid>.conf`** to remove those two **`#` lines** (and re-**apply** will not touch an existing CT’s conf), or set Notes with **`pct set <vmid> --description '…'`**.

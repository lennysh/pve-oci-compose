...
| `env` / `environment` | optional | Same field: Docker-style **`environment`** is an alias for **`env`**. YAML **mapping** (`KEY: value`), **list** of `KEY=value` strings, a single string, or a list of one-key objects / `{name:, value:}` objects (Kubernetes-style). **`defaults.env`** and service **`env`** are **merged** (service wins on duplicate keys). **OCI vztmpl:** Proxmox sets runtime env from the image **`Config.Env`** during unpack; **apply** then runs **`pct set --env`** with **image env + merged compose env** (compose keys override same names). Omit **`env`** entirely to keep only the image’s variables. |
...
- **Post-create env:** compose **`env`** is merged with the OCI image env via **`pct set --env`** (not by editing **`lxc.environment.runtime`** lines in the conf). **`lxc_config_lines`** are still appended to **`/etc/pve/lxc/<vmid>.conf`** (no **`#` markers** — Proxmox copies **`#` comment lines from the conf into the CT Notes field).
